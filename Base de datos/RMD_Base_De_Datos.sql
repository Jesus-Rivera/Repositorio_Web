...
-- Schema RMD
-- -----------------------------------------------------
CREATE DATABASE IF NOT EXISTS `RMD` DEFAULT CHARACTER SET utf8 ;
USE `RMD` ;
...
CREATE TABLE IF NOT EXISTS `RMD`.`Cuestionario` (
  ...
  INDEX `fk_Cuestionario_Pregunta1_idx` (`Pregunta_idPregunta` ASC),
  CONSTRAINT `fk_Cuestionario_Archivo1`
    FOREIGN KEY (`Archivo_idArchivo`)
    REFERENCES `RMD`.`Archivo` (`idArchivo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Cuestionario_Pregunta1`
    FOREIGN KEY (`Pregunta_idPregunta`)
    REFERENCES `RMD`.`Pregunta` (`idPregunta`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
